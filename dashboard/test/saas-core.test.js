...
test('schema migration is additive and normalizes roles', () => {
  ...
  for (const key of ['wallets', 'ledger', 'paymentIntents', 'paymentEvents', 'supportTickets', 'supportMessages', 'auditEvents', 'presets', 'byonConnections', 'hvacJobs', 'hvacSettings', 'demoLinks', 'callbackJobs', 'phoneNumbers', 'providerResources', 'calls', 'knowledgeEntries', 'integrationWebhooks', 'campaigns', 'campaignLeads', 'workflows', 'leads', 'callJobs', 'employees']) assert.ok(Array.isArray(migrated[key]));
});
